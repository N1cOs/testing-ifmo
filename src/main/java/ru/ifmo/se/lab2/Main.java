...
public class Main {
    ...
    private final static String CSC_MODULE = "csc";
    ...
    private static MathFunction getTargetFunction(String[] logModules, List<Writer> writers) throws IOException {
        ...
        for (var module : modules) {
            switch (module) {
                ...
                case CSC_MODULE: {
                    var file = new FileWriter(CSC_MODULE);
                    writers.add(file);
                    csc = builder.csc(file);
                    break;
                }
                case FUNC_MODULE: {
                    logFunc = true;
        ...
        MathFunction func = new TargetFunction(new Calculator(cos, tan, csc, log));
        if (logFunc) {
